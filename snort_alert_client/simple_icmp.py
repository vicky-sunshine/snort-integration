...
def main():
    for msg in snort_listener.start_recv("/tmp/snort_alert"):
        ...
        # Pull out fragment information (flags and offset all packed into off
        # field, so use bitmasks)
        ...
        payload = json.dumps({'srcIP': ip_to_str(ip.src),
                              'dstIP': ip_to_str(ip.dst),
                              'protocol': 'ICMP'})

        r = requests.put('http://192.168.2.160:8080/api/firewall/acl',
                         data=payload)
